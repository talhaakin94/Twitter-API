...
class TweetControllerTest {
    ...
    @Test
    void findAllReact() throws Exception {
        Tweet tweet = new Tweet();
        tweet.setId(1L);
        tweet.setMessage("hi");
        User user = new User();
        ...
        tweets.add(tweet);
        when(tweetService.findAll()).thenReturn(tweets);
        when(retweetService.findByTweetIdAndUserId(1L, 1L)).thenReturn(retweet);
        mockMvc.perform(get("/tweet/react/{userId}", 1L)
                        .with(csrf())
                        .with(user("user@gmail.com"))
                        .accept(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andExpect(MockMvcResultMatchers.jsonPath("$[0].message").value("hi"));
        verify(tweetService).findAll();
        verify(retweetService, atLeastOnce()).findByTweetIdAndUserId(1L, 1L);
    }
