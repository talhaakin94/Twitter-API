...
class RetweetControllerTest {
    ...
    void retweet() throws Exception {
        ...
        when(retweetService.retweet(1L, 1L)).thenReturn(retweet);
        mockMvc.perform(get("/retweet/{tweetId}/{userId}", 1L, 1L)
                        .with(csrf())
                        .with(user("user@gmail.com"))
                .accept(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andExpect(MockMvcResultMatchers.jsonPath("$.tweetResponse.userName").value("user"))
                .andExpect(MockMvcResultMatchers.jsonPath("$.tweetResponse.message").value("hi"));
        verify(retweetService).retweet(1L, 1L);
    }
    @Test
    void deleteRetweet() throws Exception {
        Tweet tweet = new Tweet();
        tweet.setId(1L);
        tweet.setMessage("hi");
        User user = new User();
        ...
        tweet.setLikes(likes);
        tweet.setRetweets(retweets);
        when(retweetService.delete(1L)).thenReturn(retweet);
        mockMvc.perform(delete("/retweet/{id}", 1L)
                        .with(csrf())
                        .with(user("user@gmail.com"))
                        .accept(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andExpect(MockMvcResultMatchers.jsonPath("$.tweetResponse.userName").value("user"))
                .andExpect(MockMvcResultMatchers.jsonPath("$.tweetResponse.message").value("hi"));
        verify(retweetService).delete(1L);
    }
